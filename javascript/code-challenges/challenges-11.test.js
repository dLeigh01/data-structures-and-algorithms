...
const totalSum = (input) => {
  // Solution code here...
  let total = 0;
  input.forEach(arr => total += arr.reduce((a,b) => a + b));
  return total;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let res = input.map(arr => arr.filter(element => typeof(element) === 'number'));
  let filtered = [];
  for (let i in res) {
    filtered.push(res[i].filter(num => !(num%5)).map(element => Math.pow(2, element)));
  }
  return filtered;
};
...
let findShortest = (data) => {
  // Solution code here...
  data.forEach(() => data.sort((a,b) => a.height - b.height));
  return data[0].name;
};
